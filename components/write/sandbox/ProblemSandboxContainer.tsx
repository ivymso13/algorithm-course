    ...
    <section className="rounded-2xl border border-blue-200 bg-white shadow-xs overflow-hidden">
      ...
      {isOpen && (
        <div className="p-4 sm:p-5">
          {problemType === "12coins" && (
            <CoinsSandbox key={problemType} onCopyHistory={onCopyHistory} />
          )}

          {problemType === "card" && (
            <CardsSandbox key={problemType} onCopyHistory={onCopyHistory} />
          )}

          {problemType === "josephus" && (
            <JosephusSandbox key={problemType} onCopyHistory={onCopyHistory} />
          )}

          {problemType === "pancake" && (
            <PancakeSandbox key={problemType} onCopyHistory={onCopyHistory} />
          )}
        </div>
      )}
    </section>
