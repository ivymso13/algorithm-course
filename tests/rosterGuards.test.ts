...
test("assertNoDuplicateStudentId: rejects a student ID already used by another row", () => {
  const rows = [
    { id: 1, studentId: "10111", studentKey: "10111 박OO" },
    { id: 2, studentId: "10113", studentKey: "10113 박OO" },
  ];
  assert.throws(
    () => assertNoDuplicateStudentId(rows, { studentId: "10111", studentKey: "10111 다른이름" }),
    RosterDuplicateError
  );
});

test("assertNoDuplicateStudentId: rejects a studentKey already used by another row, even with a different student ID", () => {
  // studentKey is derived from studentId+name, but the check is defensive
  // about both fields independently rather than assuming one implies the
  // other.
  const rows = [{ id: 1, studentId: "10111", studentKey: "10111 박OO" }];
  assert.throws(
    () => assertNoDuplicateStudentId(rows, { studentId: "99999", studentKey: "10111 박OO" }),
    RosterDuplicateError
  );
});

test("assertNoDuplicateStudentId: allows a student ID/studentKey with no conflict", () => {
  const rows = [{ id: 1, studentId: "10111", studentKey: "10111 박OO" }];
  assert.doesNotThrow(() => assertNoDuplicateStudentId(rows, { studentId: "10999", studentKey: "10999 김OO" }));
});
...
test("assertNoDuplicateStudentId: excludeId lets an edit compare against every OTHER row without tripping on itself", () => {
  ...
  assert.doesNotThrow(() =>
    assertNoDuplicateStudentId(rows, { studentId: "10111", studentKey: "10111 박OO" }, 1)
  );
  assert.throws(
    () => assertNoDuplicateStudentId(rows, { studentId: "10111", studentKey: "10111 박OO" }, 2),
    RosterDuplicateError
  );
});

test("assertNoDuplicateStudentId: rejects reusing a DEACTIVATED row's student ID — deactivated rows still count", () => {
  // A soft-deleted student can already own real history (submissions/votes/
  // attempts). Handing their student ID to a different roster row would mix
  // that history into the new row's identity, so the check must not filter
  // by `active` — the caller is expected to pass every row in the course,
  // deactivated included.
  const rowsIncludingDeactivated = [{ id: 1, studentId: "10111", studentKey: "10111 박OO" }];
  assert.throws(
    () => assertNoDuplicateStudentId(rowsIncludingDeactivated, { studentId: "10111", studentKey: "10111 새이름" }),
    RosterDuplicateError
  );
});
